...
GO
CREATE TABLE Procedimentos.dbo.REQUESTIA_ACTION
(
    REQUEST VARCHAR(20)
    , SEQUENCE;ACTTYPE;COSTTYPE;CURRANAL;CURRANLGRP;LASTANAL;LASTANLGRP;FROMANAL;FROMANLGRP;CURRSTATUS;FROMSTATUS;CURRPRIOR;FROMPRIOR;CLIENTAPROV;ACTDATE;ACTCOST;HOURS;MINUTES;ROUTED;STCHANGED;BILLING;DESCRIPT;COMPENSATION;MQI_PROFILEID;MQI_VERIFIED;PACKAGE;TOTALATTACH;SEVENT;INSERTDATE;UPDATEDATE;ACTCODE;CURRAPPROVCODE;CURRAPPROVANLTYPE;CURRAPPROVANALYST;CURRAPPROVCLITYPE;CURRAPPROVCLIENT;CURRAPPROVCLIEXACTOUNIT;CURRAPPROVOUNIT;CURRAPPROVMLEVEL;SYSTEMUSED;FROMSYSTEM;REQPARENT;SERVORDER;EVENTSERVICE

CSI000001;1;Mudanca de status;;Fin - Contas a Pagar;1;Fin - Contas a Pagar;1;admin;0;Aguard Atend Cont Pag;Aguardando atendimento;Baixa;Baixa;;20180601100221773;0;0;0;0;1;0;"Monitor mudou automaticamente o status do chamado de ""Aguardando atendimento"" para ""Aguard Atend Cont Pag"" (CAP_ADI_SOL1_00002)";0;0;0;MONITOR;0;CAP_ADI_SOL1_00002;20180601100221887;20180601100221887;"";"";"";"";"";"";"";"";"";"";"";"";"";""
)

/* INDICES */
CREATE INDEX IDX_CLOSED ON dbo.REQUESTIA_REQUESTS (CLOSED)
GO
CREATE INDEX IDX_NOCOMPUTE ON dbo.REQUESTIA_REQUESTS (NOCOMPUTE)
GO
...
GO
CREATE INDEX IDX_STATUS ON dbo.REQUESTIA_REQUESTS (STATUS)
GO
...
AS
    BEGIN
        ...
        -- NOTE: CONSIDERING 08 BUSSINESS HOURS PER DAY
...
AS
    ...
    IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQUESTIA_REQUESTS')
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_REQUESTS;
    END

    IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQUESTIA_QSESSION')
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_QSESSION;
    END

    IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQUESTIA_QANSWER')
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_QANSWER;
    END

    IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQUESTIA_QTABLEANSWER')
    BEGIN
    ...
    IF(@dayName = 'Sunday')
    BEGIN
        IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQ_WKF_CAPEXNFD')
        BEGIN
            DROP TABLE dbo.REQ_WKF_CAPEXNFD; -- AUTO CREATED BY sp_EXTRACTCAPEXREQUESTS
        END

        IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'REQ_WKF_ATUALIZACAOTISS')
        BEGIN
...
AS
    ...
    , [% SLA] = CASE
                    WHEN CONVERT(FLOAT, [RESLMINS]) + CONVERT(FLOAT, [RESPMINS]) = 0 THEN 0
                    ELSE (CONVERT(FLOAT, [RESLMINS]) + CONVERT(FLOAT, [RESPMINS]) - CONVERT(FLOAT, [RESLREMAINS])) / (CONVERT(FLOAT, [RESLMINS]) + CONVERT(FLOAT, [RESPMINS]))
                END
...
AS
    ...
    SET MARCA = CASE
            ...
            WHEN [UNIDADE ATENDIMENTO] LIKE '%01.162.143/%' THEN 'LC'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%03.516.553/%' THEN 'LC'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%04.998.429/%' THEN 'VISAO'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%05.942.307/%' THEN 'DH'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%10.526.385/%' THEN 'LC'
            ...
            WHEN [UNIDADE ATENDIMENTO] LIKE '%14.307.763/%' THEN 'OC'
            ...
            WHEN [UNIDADE ATENDIMENTO] LIKE '%LENTES DE CONTATO%' THEN 'LC'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%IPC%' THEN 'LC'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%MICROCIRURGIA%' THEN 'LC'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%LASER OCULAR%' THEN 'LC'
            ELSE 'N/D'
        END
    , [SIGLA UNIDADE] = CASE
            ...
            WHEN [UNIDADE ATENDIMENTO] LIKE '%14.307.763/0001-94%' THEN 'BAR'
            WHEN [UNIDADE ATENDIMENTO] LIKE '%14.307.763/0002-75%' THEN 'VIL'
...
AS
    ...
    IF(@dayName = 'Sunday')
    BEGIN
        SELECT REQUEST CHAMADO
            , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA
            , UPPER(RSTATUS) STATUS
            , UPPER(CLIENT) SOLICITANTE
            , UPPER(CURRANAL) ANALISTA
            , MARCA MARCA
        ...
        FROM REQUESTIA_REQUESTS R
            LEFT JOIN REQUESTIA_QANSWER A1
                ON QSESSIONFORM = A1.QSESSION AND A1.QUESTION = 'CAP_CAPEX_NFD_Projeto'
            LEFT JOIN REQUESTIA_QANSWER A2
                ON QSESSIONFORM = A2.QSESSION AND A2.QUESTION = 'CONTAS_PGTO_Forn'
            LEFT JOIN REQUESTIA_QANSWER A3
                ON QSESSIONFORM = A3.QSESSION AND A3.QUESTION = 'CONTAS_PGTO_Observacao'
            LEFT JOIN REQUESTIA_QANSWER A4
                ON QSESSIONFORM = A4.QSESSION AND A4.QUESTION = 'CONTAS_PGTO_Valor'
            LEFT JOIN REQUESTIA_QANSWER A5
                ON QSESSIONFORM = A5.QSESSION AND A5.QUESTION = 'CONTAS_PGTO_DataEmis'
            LEFT JOIN REQUESTIA_QANSWER A6
                ON QSESSIONFORM = A6.QSESSION AND A6.QUESTION = 'CONTAS_PGTO_NDocumento'
        WHERE CATEGORY = 'Contas a Pagar'
            AND PRODUCT = 'Solicitação de CAPEX'
            AND PROCESS = 'Registro de NFDs'
            AND RSTATUS NOT LIKE '%Cancel%'
    END
GO
...
AS
    ...
    BEGIN
        SELECT REQUEST CHAMADO
            ...
            , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA
            , UPPER(CLIENT) SOLICITANTE
            , UPPER(PROCESS) PROCESSO
            , UPPER(RSTATUS) SITUACAO
            , UPPER(CURRANAL) ANALISTA
            , FORMAT([% SLA], 'P') SLA
            , CONVERT(VARCHAR(10), [ULTIMA ACAO], 103) ULTIMA_ACAO
        ...
        FROM REQUESTIA_REQUESTS R
            LEFT JOIN REQUESTIA_QANSWER A1
                ON QSESSIONFORM = A1.QSESSION AND (A1.QUESTION = 'CAD_Atualiza_TISS_RazSoc' OR A1.QUESTION = 'CAD_CONVENIO_TASY' OR A1.QUESTION = 'CAD_CLIENTE_RazSocialCli')
            LEFT JOIN REQUESTIA_QANSWER A2
                ON QSESSIONFORM = A2.QSESSION AND A2.QUESTION = 'CAD_CLIENTE_DataSolici'
            LEFT JOIN REQUESTIA_QANSWER A3
                ON QSESSIONFORM = A3.QSESSION AND A3.QUESTION = 'CAD_Atualizacao_TISS_Desc'
            LEFT JOIN REQUESTIA_QANSWER A4
                ON QSESSIONFORM = A4.QSESSION AND A4.QUESTION = 'CAD_PAD_Sistema'
        WHERE CATEGORY = 'Central de Cadastros'
            AND (PRODUCT = 'Cadastro Comercial' OR PRODUCT = 'Cadastro Regra Cri Repa')
            --AND PROCESS LIKE '%TISS%'
    END
GO
DROP TABLE REQ_WKF_ATUALIZACAOTISS
...
GO
ALTER TABLE Procedimentos.dbo.REQUESTIA_REQUESTS ADD [STATUS] VARCHAR(40)


SELECT *
FROM REQUESTIA_REQUESTS R
WHERE REQUEST = 'CSI046512'
...
SELECT REQUEST CHAMADO
    , UPPER(PRODUCT) PRODUTO
    , UPPER(PROCESS) PROCESSO
    , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA
    , UPPER(RSTATUS) STATUS
    , UPPER(CLIENT) SOLICITANTE
    , UPPER(CURRANAL) ANALISTA
    , UPPER(LASTAPPROVUSER) APROVADOR
    , MARCA MARCA
    , [SIGLA UNIDADE] SIGLA
    , [UNIDADE ATENDIMENTO] UNIDADE
FROM REQUESTIA_REQUESTS R
WHERE CATEGORY = 'Central de Notas'
    AND (MARCA = 'DH' OR MARCA = 'IOF')
    AND ABERTURA >= GETDATE()-7
ORDER BY 1 DESC
    AND PRODUCT = 'Solicitação de CAPEX'
    AND PROCESS = 'Registro de NFDs'
    AND RSTATUS NOT LIKE '%Cancel%'
...
SELECT NIVEL
    ...
    , COUNT(1)
FROM REQUESTIA_REQUESTS R
WHERE CATEGORY = 'Tecnologia da Informacao'
    AND PRODUCT = 'Pixeon Smart'
    AND (MARCA = 'DH' OR MARCA = 'IOF')
    AND ABERTURA >= GETDATE()-30
GROUP BY NIVEL
    , SITUACAO
    , [SITUACAO SLA]
ORDER BY 4 DESC
...
SELECT *
FROM REQUESTIA_REQUESTS R
WHERE REQUEST = 'CSI061853'

EXECUTE sp_UPDATEREQUESTCLIENT
EXECUTE sp_EXTRACTCAPEXREQUESTS

SELECT TOP 20 *
FROM REQUESTIA_REQUESTS R
WHERE CATEGORY = 'Facilities'
    AND PRODUCT = 'Servicos'
    AND PROCESS = 'Medição de Contratos'
    AND CLOSED = 1
    AND ABERTURA >= '01/01/2019'
    AND LASTAPPROVUSER = 'sergio.queiroz'

SELECT *
FROM REQUESTIA_QANSWER A
WHERE A.QSESSION = 168961

SELECT REQUEST CHAMADO
    , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA
    , CONVERT(VARCHAR(10), RESOLUCAO, 103) ENCERRAMENTO
    , [UNIDADE ATENDIMENTO] UNIDADE
    , MARCA MARCA
    , [SIGLA UNIDADE] SIGLA
    , UPPER(A1.QANSWER) FORNECEDOR
    , A6.QANSWER CONTRATO
    , UPPER(A2.QANSWER) SERVICO
    , UPPER(A3.QANSWER) OBSERVACAO
    , CONVERT(VARCHAR(10), CONVERT(DATE, A4.QANSWER, 111), 103) VENCIMENTO
    , A5.QANSWER VALOR
INTO RQE_TMP_MEDICAOCONTRATOS
FROM REQUESTIA_REQUESTS R
    LEFT JOIN REQUESTIA_QANSWER A1
        ON QSESSIONFORM = A1.QSESSION AND A1.QUESTION = 'CONTAS_PGTO_Forne'
    LEFT JOIN REQUESTIA_QANSWER A2
        ON QSESSIONFORM = A2.QSESSION AND A2.QUESTION = 'CONTAS_PGTO_DescServProd'
    LEFT JOIN REQUESTIA_QANSWER A3
        ON QSESSIONFORM = A3.QSESSION AND A3.QUESTION = 'FAC_Observacao'
    LEFT JOIN REQUESTIA_QANSWER A4
        ON QSESSIONFORM = A4.QSESSION AND A4.QUESTION = 'CONTAS_PGTO_Vencimento'
    LEFT JOIN REQUESTIA_QANSWER A5
        ON QSESSIONFORM = A5.QSESSION AND A5.QUESTION = 'CONTAS_PGTO_Valorr'
    LEFT JOIN REQUESTIA_QANSWER A6
        ON QSESSIONFORM = A6.QSESSION AND A6.QUESTION = 'CONTAS_PGTO_Contrato'
WHERE CATEGORY = 'Facilities'
    AND PRODUCT = 'Servicos'
    AND PROCESS = 'Medição de Contratos'
    AND CLOSED = 1
    AND ABERTURA >= '01/01/2019'--GETDATE()-90
    AND LASTAPPROVUSER = 'sergio.queiroz'

DROP TABLE RQE_TMP_MEDICAOCONTRATOS

SELECT *
FROM RQE_TMP_MEDICAOCONTRATOS
ORDER BY FORNECEDOR, SERVICO, VENCIMENTO
...
SELECT TOP 100 REQUEST
    ...
    , *
FROM REQUESTIA_REQUESTS R
WHERE RESLMINS = RESLREMAINS
AND CLOSED = 0
--WHERE REQUEST = 'CSI086576'
WHERE REQUEST IN ('CSI086990', 'CSI086987', 'CSI002183', 'CSI086576', 'CSI076689')


EXECUTE dbo.sp_CALCULATEREQUESTS
...
SELECT RSTATUS
    ...
    , COUNT(1)
FROM REQUESTIA_REQUESTS R
WHERE REQUEST IN ('CSI071583', 'CSI077930', 'CSI081951', 'CSI085488', 'CSI074064', 'CSI070353')
GROUP BY RSTATUS
ORDER BY 3 DESC

SELECT ROW_NUMBER() OVER (ORDER BY REQUEST) ITEM
    , *
FROM REQUESTIA_REQUESTS R
WHERE REQUEST IN ('CSI071583', 'CSI077930', 'CSI081951', 'CSI085488', 'CSI074064', 'CSI070353')

SELECT ROW_NUMBER() OVER (ORDER BY CHAMADO DESC) ITEM
    , *
FROM REQ_WKF_CAPEXNFD C
WHERE CONVERT(DATE, ABERTURA, 103) >= GETDATE()-7
ORDER BY CHAMADO DESC

SELECT ROW_NUMBER() OVER (ORDER BY REQUEST DESC) ITEM
    , REQUEST CHAMADO
    , UPPER(PRODUCT) PRODUTO
    , UPPER(PROCESS) PROCESSO
    , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA
...
SELECT CONVERT(VARCHAR(10), GETDATE()-7, 103)

sp_help REQUESTIA_REQUESTS

LASTSTATUS = DATA EXECUCAO ULTIMO STATUS
NOCOMPUTE = 1 (CHAMADOS QUE PARAM SLA)
RESLTIME = TEMPO SLA CHAMADO

TEMPO PARADO STATUS = HOJE - LASTSTATUS WHERE NOCOMPUTE = 1
NOVO SLA = RESLTIME + TEMPO PARADO STATUS
24H  + 40H = 64H
...
SELECT R.REQUEST
    ...
    , R.RESOLUCAO
    , R.[ULTIMA ACAO]
    , DATEDIFF(day, R.[ULTIMA ACAO], GETDATE())
    , R.[ULTIMO ANALISTA]
    , R.REABERTURA
    , *
FROM REQUESTIA_REQUESTS R
--WHERE R.RESLREMAINS < 0
WHERE R.REQUEST IN ('CSI001747', 'CSI001719', 'CSI094706')
...
EXECUTE sp_EXTRACTTISSREQUESTS

SELECT ROW_NUMBER() OVER (ORDER BY CHAMADO DESC) ITEM
    , *
FROM dbo.REQ_WKF_ATUALIZACAOTISS
ORDER BY CHAMADO DESC
...
ALTER TABLE OFT_CONSULTA_RETORNO_INCREMENT ADD CLASSIF_AGENDA VARCHAR(50)

SELECT TOP 10 *
FROM OFT_CONSULTA_RETORNO R
WHERE DATA_ATENDIMENTO BETWEEN CONVERT(VARCHAR, '25/12/2018',103) AND CONVERT(VARCHAR, '17/10/2019', 103)

SELECT TOP 10 *
FROM OFT_CONSULTA_RETORNO R
WHERE AGENDADO_PARA_DIA BETWEEN CONVERT(VARCHAR, '25/12/2018', 103) AND CONVERT(VARCHAR, '17/10/2019', 103)

SELECT MIN(DATA_ATENDIMENTO)
    , MAX(DATA_ATENDIMENTO)
    , MIN(AGENDADO_PARA_DIA)
    , MAX(AGENDADO_PARA_DIA)
FROM OFT_CONSULTA_RETORNO R

sp_help OFT_CONSULTA_RETORNO_INCREMENT

SELECT TOP 10 *
FROM PROJESI_ZERO_FALTAS

SELECT TOP 100 *
FROM OFT_CONSULTA_RETORNO_INCREMENT

SELECT ROW_NUMBER() OVER (ORDER BY CHAMADO DESC) ITEM
    , *
FROM REQ_WKF_CAPEXNFD C
WHERE CONVERT(DATE, ABERTURA, 103) >= GETDATE()-30
ORDER BY CHAMADO DESC
...
WHERE CATEGORY = 'Tecnologia da Informacao'
AND CLOSED = 0
AND ABERTURA < GETDATE() - 45
AND ABERTURA < CAST('01 '+ RIGHT(CONVERT(CHAR(11),GETDATE(),113),8) AS datetime)
ORDER BY ABERTURA


SELECT CAST('01 '+ RIGHT(CONVERT(CHAR(11),GETDATE(),113),8) AS datetime)
WHERE REQUEST IN ('CSI093186', 'CSI092854', 'CSI095217', 'CSI091733')

SELECT DATENAME(m, ABERTURA) MES
    , MONTH(ABERTURA) M
    , MARCA
    , STATUS
    , COUNT(1) QTDE
...
WHERE CATEGORY = 'Tecnologia da Informacao'
AND CLOSED = 0
GROUP BY DATENAME(m, ABERTURA)
    , MONTH(ABERTURA)
    , MARCA
    , STATUS
ORDER BY M

SELECT ROW_NUMBER() OVER (ORDER BY [% SLA] ASC) ITEM
    , FORMAT([TEMPO ULTIMA ACAO (H)] / 8, 'N0') DDU_ULTIMA_ACAO
    , REQUEST CHAMADO
    , UPPER(CURRANAL) ANALISTA
    , NIVEL
    , STATUS
    ...
    , FORMAT([% SLA], 'P') SLA
    , MARCA
    , [SIGLA UNIDADE] UNIDADE
    , UPPER(PRODUCT) PRODUTO
    , UPPER(PROCESS) PROCESSO
    , UPPER(CLIENT) CLIENTE
FROM REQUESTIA_REQUESTS R
WHERE CATEGORY = 'Tecnologia da Informacao'
    AND CLOSED = 0
    AND NIVEL = 'N1'
    AND STATUS = 'AGUARDANDO ATENDIMENTO'
ORDER BY [% SLA]
...
DECLARE @pCATEGORY VARCHAR(100);
DECLARE @SQLString nvarchar(4000);
DECLARE @ParmDefinition nvarchar(500);

/* Build the SQL string one time.*/
SET @SQLString =
     N'SELECT ROW_NUMBER() OVER (ORDER BY [ULTIMA ACAO] ASC) ITEM
        , FORMAT([TEMPO ULTIMA ACAO (H)] / 8, ''N0'') DDU_ULTIMA_ACAO
        , REQUEST CHAMADO
        , UPPER(CURRANAL) ANALISTA
        , STATUS
    ...
    WHERE [CATEGORY] = @pCATEGORY
        AND [SITUACAO] = ''ABERTO''
        AND ([RSTATUS] NOT LIKE ''%Aguard%Aprov%'')
        AND ([RSTATUS] NOT LIKE ''%Aguard%Sol%'')
    ORDER BY [ULTIMA ACAO] ASC';
SELECT @SQLString;
SET @ParmDefinition = N'@pCATEGORY varchar(100)';

/* Execute the string with the first parameter value. */
EXECUTE sp_executesql @SQLString, @ParmDefinition,
                      @pCATEGORY = 'CENTRAL DE NOTAS';


SET @pCATEGORY = 'TECNOLOGIA DA INFORMACAO'
EXEC sp_executesql N'SELECT ROW_NUMBER() OVER (ORDER BY [ULTIMA ACAO] ASC) ITEM
    , FORMAT([TEMPO ULTIMA ACAO (H)] / 8, ''N0'') DDU_ULTIMA_ACAO
    , REQUEST CHAMADO
    , UPPER(CURRANAL) ANALISTA
    , STATUS
    ...
    , UPPER(CLIENT) CLIENTE
FROM REQUESTIA_REQUESTS R
WHERE UPPER([CATEGORY]) = ''@pCATEGORY''
    AND [SITUACAO] = ''ABERTO''
    AND ([RSTATUS] NOT LIKE ''%Aguard%Aprov%'')
    AND ([RSTATUS] NOT LIKE ''%Aguard%Sol%'')
ORDER BY [ULTIMA ACAO] ASC', N'@pCATEGORY varchar', @pCATEGORY

DECLARE @pCATEGORY VARCHAR(100);
SET @pCATEGORY = 'TECNOLOGIA DA INFORMACAO';
SELECT @pCATEGORY
SELECT ROW_NUMBER() OVER (ORDER BY [ULTIMA ACAO] ASC) ITEM        , FORMAT([TEMPO ULTIMA ACAO (H)] / 8, 'N0') DDU_ULTIMA_ACAO        , REQUEST CHAMADO        , UPPER(CURRANAL) ANALISTA        , STATUS        , CONVERT(VARCHAR(10), ABERTURA, 103) ABERTURA        , CONVERT(VARCHAR(10), [ULTIMA ACAO], 103) ULTIMA_ACAO        , FORMAT([% SLA], 'P') SLA        , MARCA        , [SIGLA UNIDADE] UNIDADE        , UPPER(PRODUCT) PRODUTO        , UPPER(PROCESS) PROCESSO        , UPP


SELECT *
FROM REQ_WKF_ATUALIZACAOTISS R
WHERE R.CHAMADO = 'CSI098145'
...
SELECT REQUEST CHAMADO
        ...
        FROM REQUESTIA_REQUESTS R
            LEFT JOIN REQUESTIA_QANSWER A1
                ON QSESSIONFORM = A1.QSESSION AND A1.QUESTION = 'CAD_Atualiza_TISS_RazSoc'
            LEFT JOIN REQUESTIA_QANSWER A2
                ON QSESSIONFORM = A2.QSESSION AND A2.QUESTION = 'CAD_CLIENTE_DataSolici'
            LEFT JOIN REQUESTIA_QANSWER A3
                ON QSESSIONFORM = A3.QSESSION AND A3.QUESTION = 'CAD_Atualizacao_TISS_Desc'
            LEFT JOIN REQUESTIA_QANSWER A4
                ON QSESSIONFORM = A4.QSESSION AND A4.QUESTION = 'CAD_PAD_Sistema'
WHERE R.REQUEST = 'CSI098145'

SELECT R.QSESSIONFORM
FROM REQUESTIA_REQUESTS R
WHERE R.REQUEST = 'CSI098645'

SELECT *
FROM REQUESTIA_QANSWER A
WHERE A.QSESSION = 196327

EXECUTE dbo.sp_EXTRACTTISSREQUESTS