...
Foreach ($ComputerName in $AllComputersNames) {
    ...
    if ($Connection -eq "True") {
        $ComputerHW = Get-CimInstance -Class Win32_ComputerSystem -ComputerName COMPRAS01 |
            select Manufacturer,
        Model,
        NumberOfProcessors,
        @{Expression = {$_.TotalPhysicalMemory / 1GB}; Label = "TotalPhysicalMemoryGB"}

        $ComputerCPU = Get-CimInstance win32_processor -ComputerName COMPRAS01 |
            select DeviceID,
        Name,
        Manufacturer,
        NumberOfCores,
        ...
        $ComputerDisks = Get-CimInstance -Class Win32_LogicalDisk -Filter "DriveType=3" -ComputerName COMPRAS01 |
            select DeviceID,
        VolumeName,
        @{Expression = {$_.Size / 1GB}; Label = "SizeGB"}

        $ComputerSerial = (Get-CimInstance Win32_Bios -ComputerName COMPRAS01).SerialNumber
